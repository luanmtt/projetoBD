...
def registrar_atendimento_completo(
    ...
    try:
        ...
        session.commit()
        print(f"Atendimento {novo_atendimento.id_atendimento} cadastrado com sucesso")
...
def calcular_tempo_medio_espera(session: Session):
    ...
    stmt = (
        select(
            ...
            func.round(func.avg(tempo_espera_segundos) / 60.0, 2).label("tempo_medio_espera_minutos"),
        )
    ...
    print("Tempo médio de espera calculado com sucesso")
    return resultado
...
def reajustar_escala(
    ...
    try:
        ...
        session.commit()
        print(f"{quantidade_alterada} escala(s) foram alteradas")
